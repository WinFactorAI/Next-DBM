...
func (gui Gui) AssetUI(sess ssh.Session, user model.User) {
	...
	publicKeyAsset, ok := sess.Context().Value("publicKeyAsset").(string)
	if ok && publicKeyAsset != "" {
		asset, err := service.WorkerService.FindMyAssetByName(publicKeyAsset, nd.SSH, user.ID)
		if err != nil || asset.ID == "" {
	...
	assetsNoSort, err := service.WorkerService.FindMyAsset("", nd.SSH, "", user.ID, "", "")
	if err != nil {
		return
	}
...
func (gui Gui) handleAccessAsset(sess ssh.Session, sessionId string) (err error) {
	...
	nextT, err := term.NewNextT(ip, port, username, password, privateKey, passphrase, pty.Window.Height, pty.Window.Width, recording, pty.Term, false)
	if err != nil {
		return err
	}
	sshSession := nextT.SshSession

	writer := NewWriter(sessionId, sess, nextT)

	sshSession.Stdout = writer
	sshSession.Stdin = writer
	sshSession.Stderr = writer

	if err := nextT.RequestPty(pty.Term, pty.Window.Height, pty.Window.Width); err != nil {
		return err
	}

	if err := nextT.Shell(); err != nil {
		return err
	}
	...
	sessionForUpdate.Status = nd.Connected
