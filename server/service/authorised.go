	...
	return s.Transaction(ctx, func(ctx context.Context) error {
		...
		for _, assetId := range item.AssetIds {
			...
			authorised := model.Authorised{
				...
				SensitiveCommandGroupId: item.SensitiveCommandGroupId,
			}
...
func (s authorisedService) UpdateWebhookPushStatusById(id string, status string) error {

	u := model.Authorised{
		ID:                id,
		WebhookPushStatus: status,
	}
	return repository.AuthorisedRepository.Update(context.TODO(), &u)
