...
func (service backupService) Export() (error, *dto.Backup) {
	...
	sqls, err := repository.SqlsRepository.FindAll(ctx)
	if err != nil {
		return err, nil
	}

	translations, err := repository.TranslationsRepository.FindAll(ctx)
	if err != nil {
		return err, nil
	}
	...
	return env.GetDB().Transaction(func(tx *gorm.DB) error {
		...
		if len(backup.UserGroups) > 0 {
			for _, item := range backup.UserGroups {
				...
				if err != nil {
					if errors.Is(nd.ErrNameAlreadyUsed, err) {
						// 删除名称重复的用户组
		...
		if len(backup.Jobs) > 0 {
			for _, item := range backup.Jobs {
				if item.Func == nd.FuncCheckAssetStatusJob {
					continue
				}
		...
		if len(backup.Sqls) > 0 {
			for _, item := range backup.Sqls {
				// item.ID = utils.UUID()
				item.Owner = userIdMapping[item.Owner]
				item.Created = common.NowJsonTime()
				if err := repository.SqlsRepository.Create(ctx, &item); err != nil {
					return err
				}
			}
		}

		if len(backup.Translations) > 0 {
			for _, item := range backup.Translations {
