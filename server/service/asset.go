...
func (s assetService) CheckStatus(asset *model.Asset, ip string, port int) (bool, error) {
	...
	if "true" == attributes[nd.SocksProxyEnable] {
		socks5 := fmt.Sprintf("%s:%s", attributes[nd.SocksProxyHost], attributes[nd.SocksProxyPort])
		auth := &proxy.Auth{
			User:     attributes[nd.SocksProxyUsername],
			Password: attributes[nd.SocksProxyPassword],
		}
...
func (s assetService) FixSshMode() error {
	return repository.AssetRepository.UpdateAttrs(context.TODO(), "ssh-mode", "naive", nd.Native)
}
