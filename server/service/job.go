...
func (r jobService) ChangeStatusById(id, status string) error {
	...
	if status == nd.JobStatusRunning {
		...
		jobForUpdate := model.Job{ID: id, Status: nd.JobStatusRunning, CronJobId: int(entryID)}
		...
		jobForUpdate := model.Job{ID: id, Status: nd.JobStatusNotRunning}
		return repository.JobRepository.UpdateById(context.TODO(), &jobForUpdate)
	}
}
...
func getJob(j *model.Job) (job cron.Job, err error) {
	...
	case nd.FuncCheckAssetStatusJob:
...
func (r jobService) InitJob() error {
	...
	if len(jobs) == 0 {
		...
		for i := range jobs {
			if jobs[i].Status == nd.JobStatusRunning {
				err := r.ChangeStatusById(jobs[i].ID, nd.JobStatusRunning)
				if err != nil {
					return err
				}
			}
...
func (r jobService) Create(ctx context.Context, o *model.Job) (err error) {
	...
	if o.Status == nd.JobStatusRunning {
		j, err := getJob(o)
		if err != nil {
			return err
		}
...
func (r jobService) DeleteJobById(id string) error {
	...
	if job.Status == nd.JobStatusRunning {
		if err := r.ChangeStatusById(id, nd.JobStatusNotRunning); err != nil {
			return err
		}
	}
...
func (r jobService) UpdateById(m *model.Job) error {
	...
	if job.Status == nd.JobStatusRunning {
		if err := r.ChangeStatusById(m.ID, nd.JobStatusNotRunning); err != nil {
			return err
		}
		if err := r.ChangeStatusById(m.ID, nd.JobStatusRunning); err != nil {
			return err
		}
	}
