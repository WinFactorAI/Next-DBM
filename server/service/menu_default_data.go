...
var DefaultMenu = []*model.Menu{
	...
	model.NewMenu("dbm-asset", "DB资产", "resource",
		model.NewPermission("GET", "/dbm-assets/paging"),
		model.NewPermission("GET", "/tags"),
	),
	model.NewMenu("sqls", "脚本管理", "resource",
		model.NewPermission("GET", "/sqls/paging"),
		model.NewPermission("GET", "/tags"),
	),
	...
	model.NewMenu("sensitive-command", "敏感指令", "resource",
		model.NewPermission("GET", "/sensitive-commands/paging"),
		model.NewPermission("POST", "/sensitive-commands"),
		model.NewPermission("PUT", "/sensitive-commands/:id"),
		model.NewPermission("DELETE", "/sensitive-commands/:id"),
	),

	model.NewMenu("trigger-command", "触发指令", "resource",
		model.NewPermission("GET", "/trigger-commands/paging"),
		model.NewPermission("POST", "/trigger-commands"),
		model.NewPermission("PUT", "/trigger-commands/:id"),
		model.NewPermission("DELETE", "/trigger-commands/:id"),
	),
	...
	// Webhook 通知
	model.NewMenu("webhook", "通知推送", "resource",
		model.NewPermission("GET", "/webhooks/paging"),
	),
	model.NewMenu("webhook-add", "增加", "webhook",
		model.NewPermission("POST", "/webhooks"),
	),
	model.NewMenu("webhook-edit", "修改", "webhook",
		model.NewPermission("PUT", "/webhooks/:id"),
	),
	model.NewMenu("webhook-del", "删除", "webhook",
		model.NewPermission("DELETE", "/webhooks/:id"),
	),
	...
	model.NewMenu("git-manager", "版本管理", "root"),
	model.NewMenu("git", "版本管理", "git-manager",
		model.NewPermission("GET", "/git/paging"),
		model.NewPermission("GET", "/tags"),
	),

	model.NewMenu("build-manager", "构建管理", "root"),
	model.NewMenu("build", "构建管理", "build-manager",
		model.NewPermission("GET", "/build/paging"),
		model.NewPermission("GET", "/tags"),
	),
	...
	// SQL执行日志
	model.NewMenu("sql-log", "执行日志", "log-audit",
		model.NewPermission("GET", "/sql-logs/paging"),
	),
	model.NewMenu("sql-log-del", "删除", "sql-log",
		model.NewPermission("DELETE", "/sql-logs/:id"),
	),
	model.NewMenu("sql-log-clear", "清空", "sql-log",
		model.NewPermission("POST", "/sql-logs/clear"),
	),

	// WebHook 日志
	model.NewMenu("webhook-push-log", "通知日志", "log-audit",
		model.NewPermission("GET", "/webhook-push-logs/paging"),
	),
	model.NewMenu("webhook-push-log-del", "删除", "webhook-push-log",
		model.NewPermission("DELETE", "/webhook-push-logs/:id"),
	),
	model.NewMenu("webhook-push-log-clear", "清空", "webhook-push-log",
		model.NewPermission("POST", "/webhook-push-logs/clear"),
	),
	// 操作日志
	model.NewMenu("oper-log", "操作日志", "log-audit",
		model.NewPermission("GET", "/oper-logs/paging"),
	),
	model.NewMenu("oper-log-del", "删除", "oper-log",
		model.NewPermission("DELETE", "/oper-logs/:id"),
	),
	model.NewMenu("oper-log-clear", "清空", "oper-log",
		model.NewPermission("POST", "/oper-logs/clear"),
	),
	...
	model.NewMenu("user-change-proxy-auth", "修改密码", "user",
		model.NewPermission("POST", "/users/:id/change-proxy-auth"),
	),
	...
	model.NewMenu("user-proxy-asset", "获取代理信息", "user-authorised-asset",
		model.NewPermission("GET", "/authorised/:id"),
	),
	...
	model.NewMenu("sensitive-command-group", "敏感策略", "ops",
		model.NewPermission("GET", "/sensitive-command-groups/paging"),
	),
	model.NewMenu("sensitive-command-group-detail", "详情", "sensitive-command-group",
		model.NewPermission("GET", "/sensitive-command-groups/:id"),
	),
	model.NewMenu("sensitive-command-group-add", "增加", "sensitive-command-group",
		model.NewPermission("POST", "/sensitive-command-groups"),
	),
	model.NewMenu("sensitive-command-group-edit", "修改", "sensitive-command-group",
		model.NewPermission("GET", "/sensitive-command-groups/:id"),
		model.NewPermission("PUT", "/sensitive-command-groups/:id"),
	),
	model.NewMenu("sensitive-command-group-del", "删除", "sensitive-command-group",
		model.NewPermission("DELETE", "/sensitive-command-groups/:id"),
	),

	model.NewMenu("trigger-command-group", "触发策略", "ops",
		model.NewPermission("GET", "/trigger-command-groups/paging"),
	),
	model.NewMenu("trigger-command-group-detail", "详情", "trigger-command-group",
		model.NewPermission("GET", "/trigger-command-groups/:id"),
	),
	model.NewMenu("trigger-command-group-add", "增加", "trigger-command-group",
		model.NewPermission("POST", "/trigger-command-groups"),
	),
	model.NewMenu("trigger-command-group-edit", "修改", "trigger-command-group",
		model.NewPermission("GET", "/trigger-command-groups/:id"),
		model.NewPermission("PUT", "/trigger-command-groups/:id"),
	),
	model.NewMenu("trigger-command-group-del", "删除", "trigger-command-group",
		model.NewPermission("DELETE", "/trigger-command-groups/:id"),
	),

	model.NewMenu("command-filter", "命令过滤", "security",
	...
	model.NewMenu("strategy", "授权策略", "security",
...
func GetMenuNameByPermission(method, path string) string {
	// for _, menu := range DefaultMenu {
	// 	for _, perm := range menu.Permissions { // 假设菜单有 Permissions 字段
	// 		if perm.Method == method && perm.Path == path {
	// 			return menu.Name // 返回菜单名称（如 "dashboard"）
	// 			// 或返回 menu.DisplayName 如需要显示名称（如 "控制面板"）
	// 		}
	// 	}
	// }
	// return "" // 未找到匹配项
	// 构建 ID 索引加速查找
	idMenuMap := make(map[string]*model.Menu)
	for _, m := range DefaultMenu {
		idMenuMap[m.ID] = m
	}

	// 1. 查找匹配权限的当前菜单
	var currentMenu *model.Menu
	for _, menu := range DefaultMenu {
	...
	// 2. 收集层级名称
	var parts []string
	parts = append(parts, currentMenu.Name) // 当前菜单名称

	// 3. 递归查找父级
	parentId := currentMenu.ParentId
	for {
		if parentId == "" { // 根据你的设计，root 的 ParentId 为空
			break
		}

		if parentMenu, exists := idMenuMap[parentId]; exists {
			parts = append([]string{parentMenu.Name}, parts...) // 父级名称插入最前
