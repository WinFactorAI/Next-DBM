...
func (service sessionService) FixSessionState() error {
	sessions, err := repository.SessionRepository.FindByStatus(context.TODO(), nd.Connected)
	if err != nil {
		return err
	}
...
func (service sessionService) ClearOfflineSession() error {
	sessions, err := repository.SessionRepository.FindByStatus(context.TODO(), nd.Disconnected)
	if err != nil {
		return err
	}
...
func (service sessionService) DisDBSess(sessionId string, code int, reason string) {
	_ = env.GetDB().Transaction(func(tx *gorm.DB) error {
		...
		if s.Status == nd.Disconnected {
			return nil
		}

		if s.Status == nd.Connecting {
			// 会话还未建立成功，无需保留数据
...
func (service sessionService) Create(clientIp, assetId, mode string, user *model.User) (*model.Session, error) {
	...
	if asset.Owner != user.ID && nd.TypeUser == user.Type {
