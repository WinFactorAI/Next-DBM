...
func (s *workerService) GetAssetGatewayByID(assetId, userId string) (map[string]string, error) {
	...
	for _, id := range assetIdList {
		if id == assetId {
			assetGatewayMap["assetId"] = assetId
			break
		}
	}

	if len(assetGatewayMap) == 0 {
		return nil, fmt.Errorf("资产不存在")
	}

	assetGatewayMap, err = repository.AssetRepository.GetAssetGatewayByID(assetId, userId)
	if err != nil {
		return nil, err
	}
