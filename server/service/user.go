...
func (service userService) DeleteALlLdapUser(ctx context.Context) error {
	return repository.UserRepository.DeleteBySource(ctx, nd.SourceLdap)
}
...
func (service userService) ChangeProxyAuth(ids []string, proxyAuth string) error {

	proxyAuthEncrypt, err := utils.Passwder.Encrypt(proxyAuth)
	if err != nil {
		return err
	}
