...
func SetupConfig() (*Config, error) {
	...
	viper.AddConfigPath("/etc/next-dbm/")
	viper.AddConfigPath("$HOME/.next-dbm")
	viper.AddConfigPath(".")
	viper.AutomaticEnv()
	viper.SetEnvKeyReplacer(strings.NewReplacer(".", "_"))

	pflag.String("db", "sqlite", "db mode")
	pflag.String("sqlite.file", path.Join("/usr/local/next-dbm/data", "sqlite", "next-dbm.db"), "sqlite db file")
	...
	pflag.String("mysql.database", "next-dbm", "mysql database")
	...
	pflag.String("guacd.recording", "/usr/local/next-dbm/data/recording", "")
	pflag.String("guacd.drive", "/usr/local/next-dbm/data/drive", "")
	pflag.String("guacd.git", "/usr/local/next-dbm/data/git", "")
	pflag.String("guacd.build", "/usr/local/next-dbm/data/build", "")
	...
	pflag.Bool("proxy.enable", true, "true or false")
	...
	guacdGit, err := homedir.Expand(viper.GetString("guacd.git"))
	if err != nil {
		return nil, err
	}

	guacdBuild, err := homedir.Expand(viper.GetString("guacd.build"))
	if err != nil {
		return nil, err
	}
	...
	if err := utils.MkdirP(config.Guacd.Git); err != nil {
		panic(fmt.Sprintf("创建文件夹 %v 失败: %v", config.Guacd.Git, err.Error()))
	}
	if err := utils.MkdirP(config.Guacd.Build); err != nil {
		panic(fmt.Sprintf("创建文件夹 %v 失败: %v", config.Guacd.Build, err.Error()))
	}
