...
func (api AccountApi) LoginEndpoint(c echo.Context) error {
	...
	if user.Status == nd.StatusDisabled {
...
func (api AccountApi) ChangeProxyAuthEndpoint(c echo.Context) error {
	if config.GlobalCfg.Demo {
		return Fail(c, 0, "演示模式禁止修改代理密码")
	}
	account, _ := GetCurrentAccount(c)

	var changeProxyAuth dto.ChangeProxyAuth
	if err := c.Bind(&changeProxyAuth); err != nil {
		return err
	}

	if len(changeProxyAuth.NewProxyAuth) < 7 {
		return Fail(c, -1, "您输入的密码过短")
	}

	if len(changeProxyAuth.NewProxyAuth) > 100 {
		return Fail(c, -1, "您输入的密码过长")
	}

	proxyAuthEncrypt, err := utils.Passwder.Encrypt(changeProxyAuth.NewProxyAuth)
	if err != nil {
		return err
	}

	u := &model.User{
		ProxyAuth: string(proxyAuthEncrypt),
		ID:        account.ID,
	}
