...
func (api SessionApi) SessionPagingEndpoint(c echo.Context) error {
	...
	for i := 0; i < len(items); i++ {
		if status == nd.Disconnected && len(items[i].Recording) > 0 {

			var recording string
			if items[i].Mode == nd.Native || items[i].Mode == nd.Terminal {
...
func (api SessionApi) SessionUploadEndpoint(c echo.Context) error {
	...
	_ = service.StorageLogService.Save(context.Background(), s.AssetId, sessionId, account.ID, nd.StorageLogActionUpload, remoteFile)
	...
	if "ssh" == s.Protocol {
		...
		sftpClient := nextSession.NextDBM.SftpClient
		// 文件夹不存在时自动创建文件夹
...
func (api SessionApi) SessionDownloadEndpoint(c echo.Context) error {
	...
	// 记录日志
	account, _ := GetCurrentAccount(c)
	_ = service.StorageLogService.Save(context.Background(), s.AssetId, sessionId, account.ID, nd.StorageLogActionDownload, file)
	...
	if "ssh" == s.Protocol {
		...
		dstFile, err := nextSession.NextDBM.SftpClient.Open(file)
		if err != nil {
			return err
		}
...
func (api SessionApi) SessionLsEndpoint(c echo.Context) error {
	...
	if "ssh" == s.Protocol {
		...
		if nextSession.NextDBM.SftpClient == nil {

		}

		fileInfos, err := nextSession.NextDBM.SftpClient.ReadDir(remoteDir)
		if err != nil {
			return err
		}
...
func (api SessionApi) SessionMkDirEndpoint(c echo.Context) error {
	...
	// 记录日志
	account, _ := GetCurrentAccount(c)
	_ = service.StorageLogService.Save(context.Background(), s.AssetId, sessionId, account.ID, nd.StorageLogActionMkdir, remoteDir)
	...
	if "ssh" == s.Protocol {
		...
		if err := nextSession.NextDBM.SftpClient.Mkdir(remoteDir); err != nil {
			return err
		}
		return Success(c, nil)
	} else if "rdp" == s.Protocol {
...
func (api SessionApi) SessionRmEndpoint(c echo.Context) error {
	...
	// 记录日志
	account, _ := GetCurrentAccount(c)
	_ = service.StorageLogService.Save(context.Background(), s.AssetId, sessionId, account.ID, nd.StorageLogActionRm, file)
...
func (api SessionApi) SessionRenameEndpoint(c echo.Context) error {
	...
	// 记录日志
	account, _ := GetCurrentAccount(c)
	_ = service.StorageLogService.Save(context.Background(), s.AssetId, sessionId, account.ID, nd.StorageLogActionRename, oldName)
...
func (api SessionApi) SessionRecordingEndpoint(c echo.Context) error {
	...
	var recording string
	if s.Mode == nd.Native || s.Mode == nd.Terminal {
