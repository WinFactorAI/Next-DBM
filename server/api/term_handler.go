...
func NewTermHandler(userId, assetId, sessionId string, isRecording bool, ws *websocket.Conn, nextT *term.NextT) *TermHandler {
...
func (r *TermHandler) readFormTunnel() {
	for {
		...
		case <-r.ctx.Done():
			return
		default:
			rn, size, err := r.nextT.StdoutReader.ReadRune()
			if err != nil {
				return
			}
			if size > 0 {
...
func (r *TermHandler) writeToWebsocket() {
	for {
		...
		case <-r.tick.C:
			...
			// 录屏
			if r.isRecording {
				_ = r.nextT.Recorder.WriteData(s)
			}
			// 监控
...
func (r *TermHandler) Write(input []byte) error {
	// 正常的字符输入
	_, err := r.nextT.Write(input)
	return err
}

func (r *TermHandler) WindowChange(h int, w int) error {
	return r.nextT.WindowChange(h, w)
}

func (r *TermHandler) SendRequest() error {
	_, _, err := r.nextT.SshClient.Conn.SendRequest("bussiness@aiputing.com", true, nil)
	return err
}
