...
func (r userGroupRepository) FindAll(c context.Context) (o []model.UserGroup, err error) {
	err = r.GetDB(c).Table("user_groups").Where("parent_id = 0").Find(&o).Error
	return
}

func (r userGroupRepository) Find(c context.Context, pageIndex, pageSize int, name, parentId, order, field string) (o []model.UserGroupForPage, total int64, err error) {
	...
	if len(parentId) > 0 {
		db = db.Where("user_groups.parent_id = ?", parentId)
		dbCounter = dbCounter.Where("user_groups.parent_id = ?", parentId)
	}

	// if len(parentId) == 0 && len(name) == 0 {
	// 	db = db.Where("user_groups.parent_id in (select value from dict_data where status = 0 and type = 'user_group_root')")
	// 	dbCounter = dbCounter.Where("user_groups.parent_id in (select value from dict_data where status = 0 and type = 'user_group_root')")
	// }
...
func (r userGroupRepository) FindByParentId(c context.Context, parentId string) (o []model.UserGroupForPage, err error) {
	err = r.GetDB(c).Table("user_groups").Where("parent_id = ?", parentId).Find(&o).Error
	return
}

func (r userGroupRepository) FindUserGroupByParentId(c context.Context, parentId string) (o []model.UserGroup, err error) {
	err = r.GetDB(c).Table("user_groups").Where("parent_id = ?", parentId).Find(&o).Error
	return
}
