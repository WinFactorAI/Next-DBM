...
func (r userRepository) Find(c context.Context, pageIndex, pageSize int, userGroupId, username, nickname, mail, online, loginPolicyId, order, field string) (o []model.UserForPage, total int64, err error) {
	...
	if len(userGroupId) > 0 {
		db = db.Joins("inner join user_group_members ugm on users.id = ugm.user_id")
		dbCounter = dbCounter.Joins("inner join user_group_members ugm on users.id = ugm.user_id")
		db = db.Where("ugm.user_group_id = ?", userGroupId)
		dbCounter = dbCounter.Where("ugm.user_group_id = ?", userGroupId)
	}
