...
func (r userGroupMemberRepository) DelUserGroupIdsByUserIdAndUserGroupId(c context.Context, userId string, userGroupId string) (err error) {
	// 先查询用户所在的用户
	err = r.GetDB(c).
		Table("user_group_members").
		Where("user_id = ? AND user_group_id = ?", userId, userGroupId).
		Delete(nil).Error
	return
}
