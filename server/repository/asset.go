...
func (r assetRepository) UpdateAttributes(c context.Context, assetId, protocol string, m maps.Map) error {
	...
	switch protocol {
	case "ssh":
		parameterNames = nd.SSHParameterNames
	case "rdp":
		parameterNames = nd.RDPParameterNames
	case "vnc":
		parameterNames = nd.VNCParameterNames
	case "telnet":
		parameterNames = nd.TelnetParameterNames
	case "kubernetes":
		parameterNames = nd.KubernetesParameterNames
	}
...
func (r assetRepository) FindAssetByName(c context.Context, name string, protocol string) (o model.Asset, err error) {
	query := r.GetDB(c).Where("name = ?", name)

	// 如果 protocol 非空，则添加额外的查询条件
	if protocol != "" {
...
func (r assetRepository) FindByIdAndDecrypt(c context.Context, assetId string) (model.Asset, error) {
	asset, err := r.FindById(c, assetId)
	if err != nil {
		return model.Asset{}, err
	}
	if err := r.Decrypt(&asset, config.GlobalCfg.EncryptionPassword); err != nil {
		return model.Asset{}, err
	}
	return asset, nil
}
...
func (r assetRepository) FindAssetAttrMapByAssetId(c context.Context, assetId string) (map[string]string, error) {
	...
	case "MySQL":
		parameterNames = nd.MySQLParameterNames
	case "ssh":
		parameterNames = nd.SSHParameterNames
	case "rdp":
		parameterNames = nd.RDPParameterNames
	...
	for i := range attributes {
		fmt.Println("  attributes[i].Name ", attributes[i].Name)
		attributeMap[attributes[i].Name] = attributes[i].Value
	}
	attributeMap["protocol"] = asset.Protocol
...
func (r assetRepository) GetAssetGatewayByID(assetId, userId string) (map[string]string, error) {
	assetGatewayMap := make(map[string]string)

	// if len(userId) == 0 {
	// 	return nil, fmt.Errorf("用户ID不能为空")
	// }

	asset, err := AssetRepository.FindById(context.Background(), assetId)
	if err != nil {
		return nil, err
	}

	assetGatewayMap["assetProtocol"] = asset.Protocol
	propertyPort, err := PropertyRepository.FindByName(context.TODO(), strings.ToLower(asset.Protocol)+"-proxy-port-gateway")
	if err != nil {
		return nil, fmt.Errorf("获取代理端口失败")
	}
	log.Info(" propertyPort " + propertyPort.Name + "  " + propertyPort.Value)
	assetGatewayMap["proxyPortGateway"] = propertyPort.Value

	// strings.ToLower(asset.Protocol)+
	propertyIp, err := PropertyRepository.FindByName(context.TODO(), "proxy-ip-gateway")
	if err != nil {
		return nil, fmt.Errorf("获取代理IP失败")
	}
	log.Info(" propertyIp   " + propertyIp.Name + "  " + propertyIp.Value)
	assetGatewayMap["proxyIpGateway"] = propertyIp.Value

	propertyLocalPort, err := PropertyRepository.FindByName(context.TODO(), strings.ToLower(asset.Protocol)+"-proxy-port")
	if err != nil {
		return nil, fmt.Errorf("获取代理端口失败")
	}
	log.Info(" propertyLocalPort " + propertyLocalPort.Name + "  " + propertyLocalPort.Value)
	assetGatewayMap["proxyLocalPort"] = propertyLocalPort.Value
	// 获取代理用户名密码
	...
	assetGatewayMap["username"] = user.Username + "#" + asset.Name
	log.Info(" assetGatewayMap.username " + assetGatewayMap["username"])
	if len(user.ProxyAuth) == 0 {
		return assetGatewayMap, fmt.Errorf("用户没有设置代理密码")
	}

	password, err := utils.Passwder.Decrypt(user.ProxyAuth)
	if err != nil {
		log.Fatal("Decrypt password error")
		return nil, err
	}

	assetGatewayMap["proxyAuth"] = password
	return assetGatewayMap, nil
}
