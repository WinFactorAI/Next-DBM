...
const AssetUserGroup = ({id, active}) => {
    ...
    const columns = [
        ...
        {
            title: i18next.t('assetUserGroup.column.userGroupName'), // 用户组名称
        ...
        {
            title: i18next.t('assetUserGroup.column.webhookPushStatus'),
            dataIndex: 'webhookPushStatus',
            key: 'webhookPushStatus',
            hideInSearch: true,
            render: (status, record, index) => {
                return <Switch checkedChildren={i18next.t('assetUserGroup.column.webhookPushStatus.enabled')} unCheckedChildren={i18next.t('assetUserGroup.column.webhookPushStatus.disabled')}
        ...
        {
            title: i18next.t('assetUserGroup.column.strategyName'), // 授权策略名称
        ...
        {
            title: i18next.t('assetUserGroup.column.sensitiveCommandGroupName'), // 敏感策略名称
        ...
        {
            title: i18next.t('assetUserGroup.column.created'), // 授权日期
            key: 'created',
            dataIndex: 'created',
            hideInSearch: true,
        },
        {
            title: i18next.t('assetUserGroup.column.option'), // 操作
            valueType: 'option',
            key: 'option',
            render: (text, record, _, action) => [
                <Show menu={'asset-authorised-user-group-del'} key={'unbind-acc'}>
                    ...
                    >
                        <a key='unbind' >  {i18next.t('assetUserGroup.action.remove')}</a>
                    </Popconfirm>
                    {/* <a
                        key="unbind"
                        onClick={async () => {
                    ...
                    >
                        {i18next.t('assetUserGroup.action.remove')}  
                    </a> */}
                </Show>
            ],
        },
    ...
    return (
        <ConfigProvider locale={locale}>
            <ProTable
                ...
                headerTitle={i18next.t('assetUserGroup.table.title')}
                toolBarRender={() => [
                    <Show menu={'asset-authorised-user-group-add'} key={'bind-acc'}>
                        <Button key="button" type="primary" onClick={() => {
                            setVisible(true);
                        }}>
                          {i18next.t('assetUserGroup.action.authorize')}
                        </Button>
                    </Show>
                    ,
                ]}
