...
import TriggerCommandModal from "./TriggerCommandGroupModal";
...
const TriggerCommand = () => {
    ...
    const columns = [
        ...
        {
            title: i18next.t('triggerCommandGroup.column.name'),
            dataIndex: 'name',
        }, {
            title: i18next.t('triggerCommandGroup.column.content'),
            dataIndex: 'content',
            key: 'content',
        }, 
        {
            title: i18next.t('triggerCommandGroup.column.created'),
            key: 'created',
            dataIndex: 'created',
            hideInSearch: true,
        },
        {
            title: i18next.t('triggerCommandGroup.column.operation'),
            valueType: 'option',
            key: 'option',
            render: (text, record, _, action) => [
                <Show menu={'trigger-command-group-detail'} key={'trigger-command-group-get'}>
                  <Link key="get" to={`/trigger-command-group/${record['id']}?activeKey=info`}>{i18next.t('triggerCommandGroup.button.detail')}</Link>
                </Show>,
                <Show menu={'trigger-command-group-edit'} key={'trigger-command-group-edit'}>
                    <a
                        key="edit"
                        onClick={() => {
                            setVisible(true);
                    ...
                    >
                        {i18next.t('triggerCommandGroup.button.edit')}
                    </a>
                </Show>,
                <Show menu={'trigger-command-group-del'} key={'trigger-command-group-del'}>
                    <Popconfirm
                        key={'confirm-delete'}
                    ...
                    >
                        <a key='delete' className='danger'>{i18next.t('triggerCommandGroup.button.delete')}</a>
                    </Popconfirm>
                </Show>,
            ],
        },
    ...
    const handleImportTriggerCommand = async (file) => {

        let [success, data] = await api.importTriggerCommand(file);
        if (success === false) {
            notification['error']({
                message: i18next.t('triggerCommandGroup.notification.importFailure'),
                description: data,
            });
            return false;
        }
        ...
        if (errorCount === 0) {
            notification['success']({
                message: i18next.t('triggerCommandGroup.notification.importSuccess'),
                description: i18next.t('triggerCommandGroup.import.success.description', { successCount }),
            });
        } else {
            notification['info']({
                message: i18next.t('triggerCommandGroup.notification.importComplete'),
                description: i18next.t('triggerCommandGroup.import.info.description', { successCount, errorCount }),
            });
        }
        actionRef.current.reload();
        return false;
    ...
    return (<Content className="page-container">
        ...
        <ProTable
            ...
            headerTitle={i18next.t('triggerCommandGroup.table.title')}
            toolBarRender={() => [
                <Show menu={'trigger-command-group-add'}>
                    <Button key="button" type="primary" onClick={() => {
                        setVisible(true)
                    }}>
                        {i18next.t('triggerCommandGroup.button.new')}
                    </Button>
                </Show>,
                ...
                //         <Upload
                //             maxCount={1}
                //             beforeUpload={handleImportTriggerCommand}
        ...
        <SelectingAsset
            ...
            handleOk={(rows) => {
                if (rows.length === 0) {
                    message.warning(i18next.t('triggerCommandGroup.message.selectAssetWarning'));
                    return;
                }
