...
import gitCommandApi from "../../api/trigger-command";
...
import TriggerCommandModal from "./TriggerCommandModal";

const {Content} = Layout;
const api = gitCommandApi;
...
function downloadImportExampleCsv() {
    ...
    a.download = 'trigger-command.csv';
...
const TriggerCommand = () => {
    const importExampleContent = <>
        <a onClick={downloadImportExampleCsv}>{i18next.t('triggerCommand.action.importExample')}</a>
        <div>{i18next.t('triggerCommand.action.import')}</div>
    </>
    ...
    const columns = [
        ...
        {
            title: i18next.t('triggerCommand.column.name'),
            dataIndex: 'name',
        }, {
            title: i18next.t('triggerCommand.column.content'),
            dataIndex: 'content',
            key: 'content',
        }, 
        // {
        ...
        // },
        {
            title: i18next.t('triggerCommand.column.created'),
            key: 'created',
            dataIndex: 'created',
            hideInSearch: true,
        },
        {
            title: i18next.t('triggerCommand.column.option'),
            valueType: 'option',
            key: 'option',
            render: (text, record, _, action) => [
                ...
                <Show menu={'command-edit'} key={'command-edit'}>
                    ...
                    >
                        {i18next.t('triggerCommand.action.edit')}
                    </a>
                </Show>,
                ...
                <Show menu={'command-del'} key={'command-del'}>
                    <Popconfirm
                        key={'confirm-delete'}
                        title={i18next.t('triggerCommand.action.deleteConfirm')}
                        onConfirm={async () => {
                            await api.deleteById(record.id);
                            actionRef.current.reload();
                        }}
                        okText={i18next.t('triggerCommand.action.deleteOk')}
                        cancelText={i18next.t('triggerCommand.action.deleteCancel')}
                    >
                        <a key='delete' className='danger'>{i18next.t('triggerCommand.action.delete')}</a>
                    </Popconfirm>
                </Show>,
            ],
        },
    ...
    const handleImportTriggerCommand = async (file) => {
        ...
        if (success === false) {
            notification['error']({
                message: i18next.t('triggerCommand.notification.importError'),
                description: data,
            });
            return false;
        }
        ...
        if (errorCount === 0) {
            notification['success']({
                message: i18next.t('triggerCommand.notification.importSuccess'),
                description: i18next.t('triggerCommand.notification.importInfo', {successCount}),
            });
        } else {
            notification['info']({
                message: i18next.t('triggerCommand.notification.importInfo'),
                description: i18next.t('triggerCommand.notification.importInfo', {successCount, errorCount}),
            });
        }
        actionRef.current.reload();
        return false;
    ...
    return (<Content className="page-container">
        ...
        <ProTable
            ...
            headerTitle={i18next.t('triggerCommand.table.title')}
            toolBarRender={() => [
                <Show menu={'command-add'}>
                    <Button key="button" type="primary" onClick={() => {
                        setVisible(true)
                    }}>
                        {i18next.t('triggerCommand.action.new')}
                    </Button>
                </Show>,
                <Show menu={'asset-import'}>
                    <Popover content={importExampleContent}>
                        ...
                        >
                            <Button key='import'>{i18next.t('triggerCommand.action.import')}</Button>
                        </Upload>
                    </Popover>
                </Show>,
            ]}
        ...
        <SelectingAsset
            ...
            handleOk={(rows) => {
                if (rows.length === 0) {
                    message.warning(i18next.t('triggerCommand.modal.selectAsset.warning'));
                    return;
                }
