...
const OptionPanel = forwardRef((props, ref) => {
  ...
  useEffect(() => {
    // debugLog(" ### OptionPanel initialData " ,props.optionData )
    setDataSource(props.optionData)
  },[props.optionData])

  const [engines, setEngines] = useState([]);
  useEffect(() => {
    // debugLog(" ### getDist('Engines') ",getDist('Engines'))
    setEngines(getDist('Engines'));
  },[])

  useEffect(() => {
    setEngines(getDist('Engines'));
    // debugLog(" ### dataSource ",dataSource)
    // debugLog(" ### engines ",engines)
    
    setCurrentEngines([])
    getDist('Engines').filter((item) => {
      if (item.value === dataSource.Engines) {
        // debugLog(" ## item ",item)
        setCurrentEngines(item)
      }
    });
  },[dataSource])

  const [currentEngines,setCurrentEngines] = useState([]);
  ...
  const handleEnginesChange = (value) => {
    // debugLog(` ## handleEnginesChange for  :`, value);
    setCurrentEngines([])
    engines.filter((item) => {
      if (item.value === value) {
        // debugLog(" ## item ",item)
      ...
      <Form form={form} {...formItemLayout} component={false} >
        <Form.Item className="form-item-spacing" label="引擎"  >
          <Select placeholder="请选择数据库引擎" 
              ...
              value={props.optionData.Engines}
              allowClear size='small' 
              options={engines} />
        </Form.Item>

        {currentEngines?.attrs?.map((attrObj, index) => {
              ...
              const getFilteredOptions = (distType, dependencyValue) => {
                debugLog(`getFilteredOptions for   :`, distType , dependencyValue);
                if(dependencyValue){
