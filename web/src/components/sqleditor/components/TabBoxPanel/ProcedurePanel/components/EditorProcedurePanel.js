...
function EditorProcedurePanel({ setValue, value, sqlEditorHeight,sqlEditorWidth }) {
  const {tabs,tabIndex,getTabByID} = useContext(VisibilityContext);
 
  const editableRef = React.createRef();  
  useEffect(() => {
    // debugLog('listHeight == ',  editableRef  );   
  ...
  // debugLog(" tabIndex ",tabIndex)
  const [sqlValue, setSqlValue] = useState("");
  useEffect(() => {
    // debugLog(' tabs[tabIndex].sql  == ',  tabs[tabIndex].sql);
  ...
  return (
    <div ref={editableRef} className="aceEditor-box w-full bg-gray-200 ProcedurePanel-EditorProcedurePanel">
      <MonacoEditor
        width="100%"
        height="100%"
...
export default EditorProcedurePanel;
