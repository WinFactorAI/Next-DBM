...
function EditorViewPanel({ setValue, value, sqlEditorHeight,sqlEditorWidth }) {
  ...
  const [sqlValue, setSqlValue] = useState(value);
  useEffect(() => {
    setValue(sqlValue)
  },[sqlValue]);
  ...
  return (
    <div ref={editableRef} className="aceEditor-box w-full bg-gray-200 ViewPanel-EditorPanel">
      <MonacoEditor
        width="100%"
        height="100%"
        ...
        value={sqlValue} // 使用状态作为编辑器的值
        onChange={editorDidMount => {
          setSqlValue(editorDidMount)
        }}
...
export default EditorViewPanel;
